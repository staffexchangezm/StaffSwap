...
- `wp-content/plugins/staffswap-woocommerce`: optional WooCommerce bridge with `[staffswap_upgrade]`, configurable VIP Gold plans, a hosted Lenco checkout supporting card and Zambian mobile-money collections, checkout CTA, membership activation with automatic expiry/renewal reminders for term plans, and a "Current plan" state once a member is subscribed.
