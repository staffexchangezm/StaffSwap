...
function staffswap_resource_download_url( $resource_id ) { return wp_nonce_url( add_query_arg( array( 'action' => 'staffswap_download_resource', 'resource_id' => absint( $resource_id ) ), admin_url( 'admin-post.php' ) ), 'staffswap_download_' . absint( $resource_id ) ); }
function staffswap_download_resource() { $resource_id = absint( $_GET['resource_id'] ?? 0 ); if ( ! $resource_id || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ?? '' ) ), 'staffswap_download_' . $resource_id ) ) { wp_die( 'Invalid resource request.' ); } if ( 'staff_resource' !== get_post_type( $resource_id ) ) { wp_die( 'Resource not found.' ); } $count = (int) get_post_meta( $resource_id, '_staffswap_download_count', true ); update_post_meta( $resource_id, '_staffswap_download_count', $count + 1 ); $file = get_post_meta( $resource_id, '_staffswap_resource_file', true ); wp_safe_redirect( $file ? esc_url_raw( $file ) : get_permalink( $resource_id ) ); exit; }
add_action( 'admin_post_staffswap_download_resource', 'staffswap_download_resource' ); add_action( 'admin_post_nopriv_staffswap_download_resource', 'staffswap_download_resource' );
function staffswap_resource_download_shortcode( $atts ) { $atts = shortcode_atts( array( 'id' => get_the_ID(), 'label' => 'Download resource' ), $atts, 'staffswap_resource_download' ); return '<a class="button button--primary" href="' . esc_url( staffswap_resource_download_url( $atts['id'] ) ) . '">' . esc_html( $atts['label'] ) . '</a>'; }
add_shortcode( 'staffswap_resource_download', 'staffswap_resource_download_shortcode' );