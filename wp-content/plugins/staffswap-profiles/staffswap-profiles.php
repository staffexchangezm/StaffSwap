...
function staffswap_profile_location_select( $id, $name, $selected = '' ) {
	if ( function_exists( 'staffswap_render_location_select' ) ) {
		return staffswap_render_location_select( $id, $name, $selected, false );
	}
	$locations = array(
	...
	$html = '<select id="' . esc_attr( $id ) . '" name="' . esc_attr( $name ) . '"><option value="">Select province &amp; town</option>';
	foreach ( $locations as $province => $towns ) {
...
function staffswap_save_profile_fields_secure( $user_id ) {
	...
	foreach ( array( 'man_number', 'profession', 'location', 'desired_location', 'employer', 'years_service', 'professional_license' ) as $field ) {
