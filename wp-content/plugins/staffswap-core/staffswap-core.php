...
function staffswap_refresh_listing_matches( $listing_id ) {
    ...
    $existing_candidate_ids = array_map( 'absint', (array) $wpdb->get_col( $wpdb->prepare( 'SELECT candidate_listing_id FROM ' . staffswap_db_table( 'matches' ) . ' WHERE listing_id = %d', $listing_id ) ) );
    staffswap_delete_listing_matches( $listing_id );
...
function staffswap_matches_shortcode() {
    ...
    update_user_meta( get_current_user_id(), 'staffswap_matches_last_viewed', current_time( 'mysql', true ) );
    global $wpdb;
...
// Counts matches created since the member last opened their matches list, for a header badge.
function staffswap_new_match_count( $user_id = 0 ) {
    $user_id = $user_id ? absint( $user_id ) : get_current_user_id();
    if ( ! $user_id ) { return 0; }
    global $wpdb;
    $last_viewed = get_user_meta( $user_id, 'staffswap_matches_last_viewed', true );
    $last_viewed = $last_viewed ? $last_viewed : '1970-01-01 00:00:00';
    return (int) $wpdb->get_var( $wpdb->prepare( 'SELECT COUNT(*) FROM ' . staffswap_db_table( 'matches' ) . ' m INNER JOIN ' . $wpdb->posts . ' p ON p.ID = m.listing_id WHERE p.post_author = %d AND p.post_status = %s AND m.status = %s AND m.created_at > %s', $user_id, 'publish', 'suggested', $last_viewed ) );
}
...
function staffswap_listing_card( $post_id ) {
    ...
    $name = get_the_title( $post_id ); $score = $meta['match_score']; $profession = $meta['profession'] ?: 'Professional';
    ob_start(); ?>
...
function staffswap_advanced_listings_shortcode( $atts ) {
    ...
    if ( 'experience' === $sort ) { $args['meta_key'] = '_staffswap_experience'; $args['orderby'] = 'meta_value_num'; $args['order'] = 'DESC'; } elseif ( 'verified' === $sort ) { $args['meta_key'] = '_staffswap_verified'; $args['orderby'] = 'meta_value'; $args['order'] = 'DESC'; } else { $sort = 'newest'; $args['orderby'] = 'date'; $args['order'] = 'DESC'; }
    $active_filters = array();
    foreach ( array( 'profession' => 'Profession', 'current_location' => 'Current location', 'desired_location' => 'Desired location', 'current_employer' => 'Employer' ) as $filter_key => $filter_label ) { if ( ! empty( $_GET[ $filter_key ] ) ) { $active_filters[ $filter_key ] = $filter_label . ': ' . sanitize_text_field( wp_unslash( $_GET[ $filter_key ] ) ); } }
    foreach ( array( 'verified' => 'Verified only', 'housing' => 'Housing available', 'urgent' => 'Urgent only' ) as $filter_key => $filter_label ) { if ( ! empty( $_GET[ $filter_key ] ) ) { $active_filters[ $filter_key ] = $filter_label; } }
    if ( isset( $_GET['min_experience'] ) && '' !== $_GET['min_experience'] ) { $active_filters['min_experience'] = 'Min ' . absint( $_GET['min_experience'] ) . ' yrs experience'; }
    if ( isset( $_GET['max_experience'] ) && '' !== $_GET['max_experience'] ) { $active_filters['max_experience'] = 'Max ' . absint( $_GET['max_experience'] ) . ' yrs experience'; }
    $query = new WP_Query( $args ); ob_start(); ?>
    <div class="marketplace"><aside class="panel"><h2>Filter listings</h2><form method="get"><div class="field"><label for="current_location">Current location</label><?php echo staffswap_render_location_select( 'current_location', 'current_location', $_GET['current_location'] ?? '', false ); ?></div><div class="field"><label for="desired_location">Desired location</label><?php echo staffswap_render_location_select( 'desired_location', 'desired_location', $_GET['desired_location'] ?? '', false ); ?></div><div class="field"><label for="profession">Profession</label><input id="profession" name="profession" value="<?php echo esc_attr( $_GET['profession'] ?? '' ); ?>" placeholder="e.g. Teacher"></div><div class="field"><label for="current_employer">Current employer</label><input id="current_employer" name="current_employer" value="<?php echo esc_attr( $_GET['current_employer'] ?? '' ); ?>" placeholder="e.g. General Hospital"></div><div class="filter-range"><div class="field"><label for="min_experience">Min years</label><input id="min_experience" name="min_experience" type="number" min="0" value="<?php echo esc_attr( $_GET['min_experience'] ?? '' ); ?>"></div><div class="field"><label for="max_experience">Max years</label><input id="max_experience" name="max_experience" type="number" min="0" value="<?php echo esc_attr( $_GET['max_experience'] ?? '' ); ?>"></div></div><?php foreach ( array( 'verified' => 'Verified users only', 'housing' => 'Housing available', 'urgent' => 'Urgent swaps only' ) as $key => $label ) : ?><label class="check"><input type="checkbox" name="<?php echo esc_attr( $key ); ?>" value="1" <?php checked( ! empty( $_GET[ $key ] ) ); ?>><?php echo esc_html( $label ); ?></label><?php endforeach; ?><div class="stack" style="margin-top:20px"><input type="submit" value="Apply filters"><a class="button button--outline" style="text-align:center" href="<?php echo esc_url( get_permalink() ); ?>">Clear filters</a></div></form></aside><section><div class="page-heading"><div><p class="eyebrow">THE MARKETPLACE</p><h1>Swap Listings</h1><p class="muted"><?php echo esc_html( $query->found_posts ); ?> swap requests matching your search</p></div><div class="listing-sort">Sort: <?php foreach ( array( 'newest' => 'Newest', 'experience' => 'Most experienced', 'verified' => 'Verified first' ) as $sort_key => $sort_label ) : ?><a class="<?php echo $sort === $sort_key ? 'is-active' : ''; ?>" href="<?php echo esc_url( add_query_arg( 'sort', $sort_key ) ); ?>"><?php echo esc_html( $sort_label ); ?></a><?php endforeach; ?></div></div><?php if ( $active_filters ) : ?><div class="filter-chips"><?php foreach ( $active_filters as $filter_key => $filter_label ) : ?><a class="filter-chip" href="<?php echo esc_url( remove_query_arg( $filter_key ) ); ?>"><?php echo esc_html( $filter_label ); ?> &times;</a><?php endforeach; ?><a class="filter-chip filter-chip--clear" href="<?php echo esc_url( get_permalink() ); ?>">Clear all</a></div><?php endif; ?><div class="listing-list"><?php if ( $query->have_posts() ) : while ( $query->have_posts() ) : $query->the_post(); echo staffswap_listing_card( get_the_ID() ); endwhile; wp_reset_postdata(); else : ?><div class="empty-state"><h2>No matching swaps found</h2><p class="muted">Try widening your location, employer, or experience filters.</p></div><?php endif; ?></div><?php $pagination = paginate_links( array( 'total' => max( 1, (int) $query->max_num_pages ), 'current' => max( 1, (int) get_query_var( 'paged', 1 ) ), 'mid_size' => 1, 'prev_text' => '« Previous', 'next_text' => 'Next »' ) ); if ( $pagination ) : ?><nav class="listing-pagination"><?php echo $pagination; ?></nav><?php endif; ?></section></div>
    <?php return ob_get_clean();
}
...
function staffswap_analytics_screen() {
    ...
    $pending_listings = (int) wp_count_posts( 'swap_listing' )->pending;
    ...
    $total_offers = post_type_exists( 'staffswap_offer' ) ? (int) wp_count_posts( 'staffswap_offer' )->publish : 0;
    $acceptance_rate = $total_offers ? round( ( $accepted_offers / $total_offers ) * 100 ) : 0;
    $active_members = (int) $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->usermeta} WHERE meta_key = 'staffswap_plus_active' AND meta_value = '1'" );
    ...
    $top_routes = $wpdb->get_results( "SELECT cur.meta_value AS from_location, des.meta_value AS to_location, COUNT(*) AS total FROM {$wpdb->postmeta} cur INNER JOIN {$wpdb->postmeta} des ON des.post_id = cur.post_id AND des.meta_key = '_staffswap_desired_location' INNER JOIN {$wpdb->posts} p ON p.ID = cur.post_id WHERE cur.meta_key = '_staffswap_current_location' AND p.post_type = 'swap_listing' AND p.post_status = 'publish' AND cur.meta_value <> '' AND des.meta_value <> '' GROUP BY cur.meta_value, des.meta_value ORDER BY total DESC LIMIT 5" );
    $offer_pipeline = post_type_exists( 'staffswap_offer' ) ? $wpdb->get_results( "SELECT meta_value AS status, COUNT(*) AS total FROM {$wpdb->postmeta} pm INNER JOIN {$wpdb->posts} p ON p.ID = pm.post_id WHERE pm.meta_key = '_staffswap_offer_status' AND p.post_type = 'staffswap_offer' AND p.post_status = 'publish' GROUP BY meta_value ORDER BY FIELD( meta_value, 'proposed', 'countered', 'accepted', 'completed', 'declined', 'withdrawn', 'cancelled', 'expired' )" ) : array();
    ?><div class="wrap"><h1>StaffSwap Analytics</h1><p>Marketplace activity for the last 30 days where noted.</p><div class="staffswap-analytics-grid"><div class="staffswap-analytics-card"><strong><?php echo esc_html( $listing_count ); ?></strong><span>Published listings</span></div><div class="staffswap-analytics-card"><strong><?php echo esc_html( $match_count ); ?></strong><span>Active match suggestions</span></div><div class="staffswap-analytics-card"><strong><?php echo esc_html( $accepted_offers ); ?></strong><span>Accepted offers</span></div><div class="staffswap-analytics-card"><strong><?php echo esc_html( $new_members ); ?></strong><span>New members, 30 days</span></div></div><div class="staffswap-analytics-grid"><div class="staffswap-analytics-card"><strong><?php echo esc_html( $pending_listings ); ?></strong><span>Listings awaiting review</span></div><div class="staffswap-analytics-card"><strong><?php echo esc_html( $active_members ); ?></strong><span>Active VIP Gold members</span></div><div class="staffswap-analytics-card"><strong><?php echo esc_html( $acceptance_rate ); ?>%</strong><span>Offer acceptance rate</span></div><div class="staffswap-analytics-card"><strong><?php echo esc_html( $total_offers ); ?></strong><span>Total offers sent</span></div></div><div class="staffswap-analytics-tables"><div><h2>Top professions</h2><table class="widefat striped"><thead><tr><th>Profession</th><th>Published listings</th></tr></thead><tbody><?php if ( $top_professions ) : foreach ( $top_professions as $profession ) : ?><tr><td><?php echo esc_html( $profession->profession ); ?></td><td><?php echo esc_html( $profession->total ); ?></td></tr><?php endforeach; else : ?><tr><td colspan="2">No published listing data yet.</td></tr><?php endif; ?></tbody></table></div><div><h2>Top swap routes</h2><table class="widefat striped"><thead><tr><th>From</th><th>To</th><th>Listings</th></tr></thead><tbody><?php if ( $top_routes ) : foreach ( $top_routes as $route ) : ?><tr><td><?php echo esc_html( $route->from_location ); ?></td><td><?php echo esc_html( $route->to_location ); ?></td><td><?php echo esc_html( $route->total ); ?></td></tr><?php endforeach; else : ?><tr><td colspan="3">No published listing data yet.</td></tr><?php endif; ?></tbody></table></div><div><h2>Offer pipeline</h2><table class="widefat striped"><thead><tr><th>Status</th><th>Offers</th></tr></thead><tbody><?php if ( $offer_pipeline ) : foreach ( $offer_pipeline as $stage ) : ?><tr><td><a href="<?php echo esc_url( admin_url( 'edit.php?post_type=staffswap_offer&staffswap_offer_status=' . $stage->status ) ); ?>"><?php echo esc_html( ucfirst( $stage->status ) ); ?></a></td><td><?php echo esc_html( $stage->total ); ?></td></tr><?php endforeach; else : ?><tr><td colspan="2">No swap offers sent yet.</td></tr><?php endif; ?></tbody></table></div></div></div><?php
}

function staffswap_analytics_admin_styles( $hook ) {
    if ( 'swap_listing_page_staffswap-analytics' !== $hook ) { return; }
    wp_add_inline_style( 'common', '.staffswap-analytics-grid{display:grid;grid-template-columns:repeat(4,minmax(150px,1fr));gap:16px;max-width:900px;margin:20px 0 28px}.staffswap-analytics-card{background:#fff;border:1px solid #dcdcde;padding:20px}.staffswap-analytics-card strong{display:block;font-size:28px;color:#135e96}.staffswap-analytics-card span{color:#50575e}.staffswap-analytics-tables{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:20px;max-width:1300px}@media(max-width:782px){.staffswap-analytics-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.staffswap-analytics-tables{grid-template-columns:1fr}}' );
}
